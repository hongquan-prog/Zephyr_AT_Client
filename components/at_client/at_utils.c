 ...
 * 2022-12-21     lihongquan   port to esp32
...
RT_WEAK rt_size_t at_utils_send(rt_device_t dev,
                                ...
                                rt_size_t size)
{
    return com_write(dev, buffer, size, RT_WAITING_FOREVER);
}
...
rt_size_t at_vprintf(rt_device_t device, const char *format, va_list args)
{
    ...
    return com_write(device, (unsigned char *)send_buf, last_cmd_len, RT_WAITING_FOREVER);
}
...
rt_size_t at_vprintfln(rt_device_t device, const char *format, va_list args)
{
    ...
    return com_write(device, send_buf, len, RT_WAITING_FOREVER);
}
